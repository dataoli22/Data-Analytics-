...
gs= yf.download('ril', start="2011-1-1", end= "2023-03-01")
...
#Auto arima to select optional ARIMA parameters
...
def arima_forecast(history):
    model = smapi.tsa.arima.ARIMA(history, order=(0,1,0))
    model_fit = model.fit()
...
plt.plot(dataset.iloc[size:,:], test, label='Real')
plt.plot(dataset.iloc[size:,:], predictions, color='red', label='Predicted')
...
def ema(close, period=20):
    return dataset.ewm(span=period, adjust=False).mean()
...
dataset['ema_20'] = ema(["Close"], 20)
dataset['ema_50'] = ema(["Close"], 50)
dataset['ema_100'] = ema(["Close"], 100)

dataset['rsi'] = rsi(["Close"])
dataset['macd'] = macd(["Close"])
dataset['obv'] = obv(["Close"],["Volume"])
...
arima_df = pd.DataFrame(index=dataset.index, columns=['ARIMA'])
